...
- **Design + simulation. No fiat is held and no fiat rail exists.** `issue_thbc`,
  ...
  the payment leg; compose defaults to `simulated`. Of nine invariants F3, F5, F7 and
  F9 may be described as guarantees (**F8 non-custody is violated** — IAM can decrypt
