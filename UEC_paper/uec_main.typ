...
Experiments ran on one host (Apple M2, Agave 3.1.10). Sweeps use a private `solana-test-validator`, isolating SVM semantics — account locking, compute metering — from consensus; the settle before/after, telemetry and liveness tests are repeated on a 3- and 4-member `agave-validator` cluster with real leader rotation and Tower-BFT voting @solana2019towerbft, on loopback. Throughput is *client-observed confirmed goodput*: confirmed transactions per wall-clock second from burst start to last confirmation, covering the pipeline from client signing to confirmation visibility. Every non-confirmed transaction is attributed to one class — send-rejected, guard-rejected (executed, failed a program check, fee paid), or expired — separating delivery loss from validation working as intended. Transactions are pre-signed against a cached blockhash and submitted raw without preflight or retry, then confirmed by bulk signature-status polling under a 3,000-transaction in-flight window; latency is poll-quantised, an upper bound rather than a measurement of runtime cost. CU is machine-independent, read post-hoc from transaction metadata.
...
#figure(
  ...
  budget. Read from transaction metadata; modes, PDA-deriving paths carry the
  bump ladder above them (see text).],
  tbl(
...
*Compute is also $O(1)$ in accumulated state:* run B's last 200,000 submissions cost the same against 471,160 resident accounts as the first scale's did against 161,160. The account store explains it — an update appends, and lookup is one in-memory pubkey → (slot, offset) probe — so what scales with account count (index memory, snapshot size) never reaches per-transaction cost.

*Throughput orders by lock footprint, but the mechanism does not confirm.* @tab-tps sorts every measured path by the shared writable state it touches, and that ordering — not the CU column — tracks the rate across three orders of magnitude. The causal test fails. Anchor's `mut` is a writability *requirement*, not a prohibition, so the shipped binary accepts the pre-fix footprint when the client declares `ZoneMarket` writable — one `AccountMeta`, verified on the wire. Over five repeats of 1,200 settles the writable arm returns 493 against 561 confirmed settles·s#super[−1] across eight fee payers, 447 against 446 across one. Even the shared fee-payer lock does not move throughput: conflicting transactions still land within one slot as successive entries, and at ≈120 k CU per settle the block compute budget saturates before locks bind. We therefore withdraw the 2.7–3× previously attributed to deleting this `mut`; both original figures are accounted for — validator warm-up (a session's first burst runs ≈5–6× slower than every later one) and a serial-await harness measuring its own round-trip (≈0.6 TPS reproduced as 0.69, p50 463 ms). Order entry shares the caveat: the *cheaper* instruction returned 180 TPS against telemetry's 462 under a vestigial `mut` since dropped. Re-measured post-fix, it lands all 10#super[4] orders at ≈1,000 TPS in every arm from one payer on one shard to 16 on 16 — a 5.7× spanning fix, harness and warm-up, so none of it attributes to the lock. Telemetry, holding only the gateway fee payer, sustains 234–296 TPS from 10,000 to 150,000 meters with no monotone trend, dipping to 182 in the final 200,000-meter epoch: not proportional, since one payer signs everything, and largely scale-independent, since per-meter PDAs never contend.
