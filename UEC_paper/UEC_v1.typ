...
- *Environment.* One host (Apple M2, Agave 3.1.10). Experiments A and B run on a private `solana-test-validator`, isolating SVM semantics — account locking and compute metering — from consensus; the settle before/after, the telemetry comparison and the liveness tests are repeated on a 3- and 4-member `agave-validator` cluster with real leader rotation and Tower-BFT voting @solana2019towerbft, all nodes on loopback.
