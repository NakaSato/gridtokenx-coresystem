...
_Off-chain matching reaches ~3.1 × 10#super[4] pairs/s, while a single-pair on-chain settlement costs 96,707 compute units (~48% of the default budget) — leaving headroom for batch settlement._
...
#figure(
  caption: [Measured per-instruction compute-unit cost across the on-chain programs (LiteSVM `computeUnitsConsumed`, default-feature build; `settle_offchain_match` measured on a live validator per @sec:settlement-cost), sorted by cost. Instruction names are those of the evaluated commit; `swap_grx_for_thbc` has since been replaced (see the footnote in @sec:treasury-program). Only the signature-verifying settlement path approaches half the 200,000 CU per-instruction budget (dashed); every other instruction stays well below — the measured signature of a thin settlement layer.],
  text(size: 6.5pt)[
    #show raw: set text(font: ("Courier New", "Courier"), size: 5.5pt)
    #let names = ([`update_meter_reading`], [`record_settlement_sharded`], [`burn_tokens`], [`aggregate_readings`], [`trigger_market_clearing`], [`create_sell_order`], [`match_orders`], [`update_erc_limits`], [`submit_meter_reading`], [`mint_to_wallet`], [`register_meter`], [`swap_grx_for_thbc`], [`deposit_escrow`], [`settle_offchain_match`])
...
The surplus generation-mint path, exercised end-to-end on a live validator rather than in LiteSVM, confirms the same thin-compute picture from the opposite end of the pipeline. A single-recipient `mint_generation` transaction (@sec:tx-structure) measured on-chain consumes about 4,455 CU for the idempotent associated-token-account creation and about 1,237 CU for the Token-2022 `MintTo` cross-program invocation, keeping the whole minting transaction an order of magnitude below the 200,000 CU per-instruction budget and far under the signature-verifying settlement path. As with settlement, neither compute nor the packet binds a single mint; the cost of scaling comes only from batching recipients. Its transaction fee of 10,000 lamports corresponds to the two required signatures — the platform authority, which is also the fee payer, and the mandatory REC-validator co-signer — the same two-party control that governs generation minting.
...
== Price-mechanism Comparison on Prosumer Revenue <sec:price-mechanism-revenue>
The analysis in @sec:revenue-sensitivity is a model-derived computation from fixed parameters at a single matched quantity. This section raises it to a measurement over a full simulated workload, comparing the net revenue a prosumer earns on exported surplus under three price mechanisms: a uniform-price auction that clears the whole book at the marginal ask, a discriminatory pay-as-ask CDA, and a regulated feed-in buy-back at 2.20 baht per kWh.

The datasets are generated by the Smart Meter Simulator under the full grid model (load, solar, power flow, losses, tap control) as five fleets of 80, 160, 240, 320, and 400 meters, with the prosumer share fixed at 10% of meters in every fleet, over 30 days at a 15-minute sampling interval (96 points per day). All fleets use the same seed (42) and therefore the same weather sequence (12 sunny, 8 partly cloudy, 4 overcast, 5 cloudy, 1 rainy days), with a grid-export cap of 5 kWh per prosumer per day. That cap binds on most days but not all — the actually tradeable surplus lands at 85.7–92.6% of the ceiling — so surplus volume is determined by the grid physics rather than by an arithmetic identity from the cap.

Charges are taken from the on-chain rate schedule (`TariffConfig`): a flat wheeling charge of 1.15 baht per kWh, a market fee of 25 bps, and a loss cost of 5 bps, with the buy-back exempt from wheeling as a regulated tariff. The demand side is derived from the actual consumption of the same fleet's consumers (participation share $alpha = 0.02$ over a 2.60–4.10 baht per kWh willingness-to-pay band). The result is summarized in @tbl:price-mechanism-revenue.

#figure(
  caption: [Prosumer net revenue on exported surplus under three price mechanisms, across five fleet sizes (30 days, 10% prosumers, 15-minute interval, 5 kWh/day export cap, seed 42). Values in baht. Flat wheeling 1.15/kWh, market fee 25 bps, loss 5 bps; buy-back at 2.20/kWh is exempt from wheeling.],
  text(size: 7pt)[
    #show math.equation: set text(size: 7pt)
    ...
    #table(
      ...
      table.header([*meters*], [*prosumers*], [*surplus (kWh)*], [*uniform*], [*CDA*], [*buy-back*]),
      [80],  [8],  [1111.4], [2544.70],  [2295.39],  [2445.07],
      [160], [16], [2117.7], [4848.74],  [4373.69],  [4658.89],
      [240], [24], [3083.6], [7060.28],  [6368.56],  [6783.84],
      [320], [32], [4304.9], [9856.71],  [8891.01],  [9470.77],
      [400], [40], [5322.5], [12186.59], [10992.62], [11709.43],
    )
  ],
) <tbl:price-mechanism-revenue>

Three observations follow from @tbl:price-mechanism-revenue. First, there is an inversion between gross traded value and net revenue: the CDA generates roughly 46% more gross value than the buy-back, yet returns roughly 6% #emph[less] net revenue to prosumers in every fleet. The cause is that wheeling is charged as a flat rate per unit of energy while the market fee and loss cost are charged as a proportion of value, so a flat wheeling charge is regressive in price — it deducts the same amount from a high-priced and a low-priced kWh alike. The ask price at which the CDA breaks even against the buy-back is $(2.20 + 1.15) slash 0.997 approx 3.3601$ baht per kWh, which exceeds three of the four rungs of the ask ladder in use (3.00, 3.15, 3.30, and 3.45). The uniform-price auction wins precisely because it pays every rung at the single marginal top price.

Second, the net rate per unit is independent of fleet size: all five fleets return identical values to four decimal places — uniform 2.2897, CDA 2.0653, and buy-back 2.2000 baht per kWh. Because both supply and demand scale with the fixed prosumer and consumer shares, the clearing point does not move. This demonstrates scale-invariance of the mechanism ranking rather than an effect of scale on price, with total revenue growing linearly in the prosumer count.

Third, the result is confirmed on a live chain rather than computed off-chain alone. We drove the order book of all five fleets through the `clear_auction` and `match_orders` instructions on a validator with every program deployed, then re-read the resulting chain state for verification. All 5 fleets pass, over 35 checks in total, covering the clearing price recorded in `zone_market`, the cumulative volume delta at atomic-unit precision, the price of every `TradeRecord` against its own ask, and the persistence of the inversion described above.

Three limitations of this measurement must be stated explicitly. The first concerns what the level of the ask ladder means. These are the prices actually used at settlement, which — following the matcher's price formation — are landed prices that already include the wheeling charge per @eq:clearing. Since on-chain settlement then deducts the wheeling charge from that price again, the seller receives approximately their own original ask, consistent with the pass-through character analysed in @sec:revenue-sensitivity rather than in conflict with it. All three columns of @tbl:price-mechanism-revenue are therefore comparable seller-side amounts. However, the ranking between the CDA and the buy-back depends on the #emph[level] of the chosen ask ladder, which is exogenous to the model: a ladder of 3.00–3.45 baht per kWh corresponds to seller-side asks of roughly 1.85–2.30 baht per kWh, which straddles the 2.20 buy-back rate exactly. A ladder sitting above the 3.3601 break-even would reverse that ranking. What does not depend on the ladder level is that the uniform-price auction always returns more than the CDA, which follows from the flat per-unit wheeling charge as described above. Second, the generated demand exceeds supply by 2.30–2.42×, so the book always clears in full at the highest ask — the outcome most favourable to the uniform-price auction. Under scarce demand the clearing price would fall and the ranking could change. Third, $alpha$ and the willingness-to-pay band are stylised assumptions, not values measured from real user behaviour. Full welfare measurement and participants' strategic behaviour remain future work (see @sec:discussion_limitations).
