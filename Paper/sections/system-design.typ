...
#figure(
  ...
  caption: [ลำดับเวลาการเคลียร์ตลาดของโปรแกรม oracle: ค่าอ่านที่ลงนามแล้วทยอยเข้ามาตลอดหน้าต่างรวมข้อมูล 15 นาที (900 วินาที) เมื่อหน้าต่างปิด oracle จะรวมค่าอ่าน สั่งเคลียร์ตลาด แล้วชำระคู่ที่จับคู่ได้บนเชน ทั้งนี้เป็นเส้นทาง telemetry บนเชนตามที่ออกแบบและที่วัดแบบแยกส่วนใน@sec:meter-scale ส่วนสายงานที่ติดตั้งใช้งานจริง#strong[ไม่ได้]เขียนค่าอ่านลงเชน แต่จบเส้นทาง ingest ไว้ที่ Redis Streams และถังหน้าต่างเวลาใน@fig:telemetry-dissemination แล้วจึงแตะเชนเฉพาะตอน mint และตอนชำระธุรกรรมเท่านั้น],
) <fig:clearing-window>

=== Trading: วงจรคำสั่งซื้อขายและ escrow บนเชน <sec:trading-program>
โปรแกรม trading จัดการวงจรชีวิตของคำสั่งซื้อขายและบัญชี escrow บนเชน เสริมกับเส้นทางการชำระใน@sec:settlement-model ผู้ใช้สร้างคำสั่งผ่าน create_sell_order และ create_buy_order (รวมถึง submit_limit_order และ submit_market_order) ซึ่งสร้างบัญชี Order แบบ PDA รายคำสั่งที่ผูกกับเจ้าของ ส่วนอายุคำสั่ง (expires_at) เป็นค่าที่ไคลเอนต์ส่งเข้ามา มิใช่ค่าคงที่ที่โปรแกรมประทับให้ โดยโปรแกรมตรวจเพียงว่าค่าที่รับมาต้องอยู่ในอนาคตอย่างเคร่งครัด และสงวนค่า `0` ไว้เป็นสัญลักษณ์ว่าไม่มีวันหมดอายุ ซึ่งเป็นความหมายเดียวกับที่เส้นทางชำระอ่านจาก payload ที่ลงนาม (`expires_at == 0 || now < expires_at`) จึงไม่มีคำสั่งใดที่มีชีวิตอยู่บนเส้นทางหนึ่งแต่หมดอายุบนอีกเส้นทางหนึ่ง ค่าที่ใช้จริงถูกกำหนดนโยบายในชั้น off-chain ของ Trading Service ที่ค่าเริ่มต้น 15 นาที (900 วินาที เท่ากับหนึ่งหน้าต่างเคลียร์ตลาด) และเพดาน 7 วัน ทั้งนี้คำสั่งขายต้องอ้างใบรับรอง REC ที่ยังไม่หมดอายุก่อนจึงจะสร้างได้ ซึ่งสอดคล้องกับการกำกับด้วย REC ใน@sec:governance-control-plane ก่อนเข้าสู่ตลาด ผู้ใช้ฝากโทเคนเข้าบัญชี escrow ผ่าน deposit_escrow และถอนคืนส่วนที่ไม่ถูกใช้ผ่าน withdraw_escrow โดยบัญชี escrow เป็น SPL token account ภายใต้ PDA ที่ลงนามโดย market_authority

โปรแกรมมีจุดเข้าสำหรับการชำระสองทาง และการเลือกใช้ทางใดมีนัยสำคัญ ทางแรกคือ `settle_offchain_match` ซึ่งเป็นรูปแบบไม่รับฝากทรัพย์สิน (non-custodial) ที่ใช้วิเคราะห์ตลอด@sec:settlement-model กล่าวคือเงินของแต่ละฝ่ายอยู่ในบัญชี escrow PDA ของฝ่ายนั้นเอง และทุกบัญชีที่คำสั่งแตะต้องถูกบังคับให้เท่ากับ PDA ตามหลักการที่หาได้จาก payload ที่คู่สัญญาลงนาม ส่วนทางที่สองคือ `execute_atomic_settlement` ซึ่งเป็นรูปแบบรับฝากทรัพย์สิน (custodial) ที่บัญชี escrow เป็น associated token account แบบรวมกองซึ่งเป็นของกุญแจแพลตฟอร์ม (ซึ่งเป็น market authority ด้วย) ลายเซ็นของแพลตฟอร์มเพียงลายเดียวจึงตอบสนองช่องผู้มีอำนาจของ escrow ทั้งสองช่อง และคู่สัญญาไม่มีบัญชี escrow ของตนเอง การติดตั้งที่ใช้วัดผลในบทความนี้ส่งการชำระผ่านทางแบบ custodial เพราะต้นแบบรับผู้ใช้เข้าระบบด้วยกระเป๋าที่แพลตฟอร์มดูแลให้ มิใช่กุญแจที่ผู้ใช้ถือเอง ผลที่ตามมาต้องระบุอย่างตรงไปตรงมาว่า สำหรับเส้นทางนี้ การรับประกันที่ว่าการหาที่อยู่บัญชี (address derivation) ผูกเงินไว้กับฝ่ายที่ลงนาม (@sec:settlement-model) ย่อมไม่เป็นจริง และผู้ใช้ต้องเชื่อถือเพิ่มเติมว่ากุญแจแพลตฟอร์มจะไม่จ่ายเงินออกจากกองรวมนอกเหนือจากการชำระที่จับคู่แล้ว การย้ายการติดตั้งไปใช้เส้นทาง non-custodial `settle_offchain_match` ซึ่งมีบัญชี escrow PDA รายฝ่ายและคำสั่งเติมเงินอยู่แล้ว คือการชำระหนี้ทางเทคนิคของข้อนี้ และไม่ใช่การเปลี่ยนแบบจำลองการชำระ แต่เป็นการเปลี่ยนว่าบริการชำระฝั่ง off-chain เรียกจุดเข้าใด
...
โปรแกรม governance ทำหน้าที่เป็นชั้นควบคุม (control plane) แบบ Proof of Authority บนเชน ออกแบบเป็นโปรแกรมเดียวที่รวม 22 instruction แบ่งตามหน้าที่เป็นหกระบบย่อย (กลุ่มคำสั่งหลัก 21 รายการ ร่วมกับคำสั่งสืบค้นสถิติ get_governance_stats อีกหนึ่งรายการ) จุดสำคัญเชิงสถาปัตยกรรมคือ governance ไม่ได้ทำงานโดดเดี่ยว แต่เป็นแหล่งความจริง (source of truth) ที่โปรแกรมอื่นบนเชนอ่านสถานะไปกำหนดพฤติกรรมของตน กล่าวคือ governance บันทึก "นโยบาย" ขณะที่ trading และ oracle เป็นผู้ "บังคับใช้" นโยบายนั้น ณ ขอบเขตของโปรแกรมตนเอง ระบบย่อยทั้งหกประกอบด้วย
...
- ระบบควบคุมพารามิเตอร์ (Config gates): update_governance_config (เปิด/ปิดการตรวจ REC และการโอน), update_erc_limits, set_maintenance_mode (สวิตช์หยุดทั้งระบบ), update_authority_info, set_oracle_authority และ initialize_zone_config ที่สร้างบัญชีพารามิเตอร์รายโซน ทุกคำสั่งต้องลงนามโดย authority
- ระบบใบรับรองพลังงานหมุนเวียน (REC certificates): issue_erc ตรวจตามนโยบายใน GovernanceConfig แล้วเรียก Cross-Program Invocation (CPI) ไปยัง registry เพื่อทำเครื่องหมายว่าพลังงานถูกอ้างสิทธิ์แล้ว (mark_erc_claimed) ร่วมกับ validate_erc_for_trading, transfer_erc และ revoke_erc (ตัวระบุ `erc` หมายถึงการออกใบรับรองภายใต้อำนาจ ERC ส่วนตัวใบรับรองเองคือ REC)
...
- ระบบโทเคน REC แบบทดแทนกันได้ (fungible REC): init_rec_mint สร้าง mint ของ REC หนึ่งครั้ง (1 โทเคน = 1 MWh ทศนิยม 6 ตำแหน่ง) และ retire_rec สำหรับเผาโทเคนเพื่อปลดระวางใบรับรอง ซึ่งแยกจากใบรับรอง ErcCertificate รายใบในระบบย่อยก่อนหน้า
...
#figure(
  ...
  text(size: 8pt)[
    ...
    #table(
      ...
      [`GovernanceConfig`], [`[governance_config]`], [singleton: authority, pending_authority, นโยบาย REC, maintenance flag, min_quorum_votes และตัวนับ],
...
มีประเด็นขอบเขตหนึ่งที่ต้องระบุไว้เพื่อไม่ให้กล่าวเกินกว่าที่ระบบทำงานจริง ในต้นแบบที่ติดตั้งใช้งาน เส้นทาง ingest ไม่ได้เขียนค่าอ่านลงเชน กล่าวคือ Aggregator Bridge กระจายค่าอ่านที่ตรวจแล้วลง Redis Streams รายโซนและถังหน้าต่างเวลา 15 นาที (@fig:telemetry-dissemination) ส่วนการส่งต่อไปยัง oracle บนเชนเป็นช่องทางเสริมแบบ best-effort ที่หากปลายทางใช้งานไม่ได้ต้องไม่ขวางการ ingest ระบบจึงแตะเชนที่ขอบเขตการ#strong[ชำระ] ได้แก่ การ mint พลังงานส่วนเกินเมื่อหน้าต่างปิด และการชำระธุรกรรมซื้อขาย มิใช่ต่อค่าอ่านหนึ่งรายการ เส้นทางรับค่าอ่านบนเชนที่อธิบายในหัวข้อย่อยนี้จึงถูกพัฒนาและวัดแบบแยกส่วนในฐานะการทดลองด้านการขยายขนาด (@sec:meter-scale) ซึ่งแสดงว่าบัญชี MeterState แบบ PDA รายมาตรวัดทำให้การเขียนค่าอ่านขนานกันได้ถึงระดับ 200,000 มาตรวัด แต่ไม่ใช่เส้นทางที่การรันแบบ end-to-end ใน@sec:evaluation ใช้งาน การยกเส้นทางนี้ขึ้นมาเป็นสายงานจริงย่อมมีต้นทุนบนเชนต่อค่าอ่านหนึ่งรายการ ซึ่งเป็นต้นทุนที่การออกแบบนี้ตั้งใจหลีกเลี่ยง
...
โปรแกรม treasury เป็นปลายทาง CPI ของการบันทึกการชำระจากโปรแกรม trading และเป็นแกนการเงินของระบบ การบันทึกการชำระแบบกลุ่ม (record_settlement_batch) เขียนบัญชี SettlementRecord รายคู่ (zone_id, batch_id) ที่เก็บรากเมอร์เคิล (merkle_root ขนาด 32 ไบต์) มูลค่ารวม และภาษีมูลค่าเพิ่ม (vat_amount, vat_rate_bps) เป็นข้อผูกพันสำหรับตรวจสอบย้อนหลัง โดยรากเมอร์เคิลผูกชุดธุรกรรมในกลุ่มไว้บนเชนขณะที่ใบไม้ (leaf) ของต้นไม้ถูกเก็บนอกเชน คำสั่งฐานนี้กระทบยอดรวมส่วนกลาง (total_settled_thbc) โดยตรง เพื่อรองรับการชำระพร้อมกันจำนวนมากโดยไม่ให้บัญชี treasury กลายเป็นคอขวด จึงมีคำสั่งแบบขนาน (record_settlement_batch_sharded) ที่บันทึก SettlementRecord เช่นเดิมแต่เพิ่มยอดลงบัญชีตัวสะสมรายชาร์ดแทนยอดรวมส่วนกลาง โดยแบ่งออกเป็น 16 ชาร์ด (`NUM_SETTLE_SHARDS`) ทั้งนี้หมายเลขชาร์ดของเส้นทางชำระเป็นค่าที่ผู้เรียกระบุมาเอง (`settle_shard_id`) และโปรแกรมตรวจเพียงว่าอยู่ในช่วงที่ถูกต้อง (`settle_shard_id < NUM_SETTLE_SHARDS`) มิได้อนุพันธ์จากกุญแจเหมือนการแบ่งชาร์ดของ registry สิ่งที่ปิดกั้นการเขียนผิดที่จึงเป็นการผูกบัญชีตัวสะสมเข้ากับหมายเลขนั้นผ่าน seed ของ PDA (ผู้บันทึกจึงกระจายยอดไปยังบัญชีใดตามอำเภอใจไม่ได้) มิใช่การที่ผู้เรียกเลือกชาร์ดไม่ได้ แล้วกระทบยอดเข้าศูนย์กลางภายหลังด้วย aggregate_settlement_shards ด้านการตรึงค่า stablecoin THBC คำสั่ง swap_grx_for_thbc บังคับว่าอุปทานใหม่ต้องไม่เกินทุนสำรองที่ได้รับการรับรอง (`new_supply <= attested_reserve`) และการรับรองต้องยังไม่หมดอายุ (`now - attestation_ts <= attestation_ttl`) ส่วน redeem_thbc_for_grx จำกัดการไถ่ถอนไม่ให้เกินหลักประกันในคลัง (`grx_out <= swap_vault.amount`)#footnote[คำสั่งทั้งสองถูกแทนที่ภายหลังคอมมิตที่ประเมินในบทความนี้ โดยเพดานทุนสำรองและการตรวจอายุการรับรองย้ายไปอยู่ที่ `issue_thbc` ซึ่งเป็นคำสั่งเดียวที่เพิ่มอุปทาน ส่วนคู่คำสั่งแลกเปลี่ยนสกุลเงิน `exchange_grx_for_thbc` และ `exchange_thbc_for_grx` เปลี่ยนเป็นการโอนจากคลัง THBC ที่แพลตฟอร์มถือครอง โดยไม่เปลี่ยนแปลง `thbc_supply` เพื่อมิให้สินทรัพย์ผันผวนเข้าไปอยู่ในชุดหลักประกันของหน่วยที่อ้างอิงเงินบาท ค่าต้นทุน compute ที่รายงานในบทความนี้เป็นค่าของคำสั่งตามที่ประเมิน ทั้งนี้คำสั่งที่มาแทนมีต้นทุนสูงกว่ามิใช่ต่ำกว่า จึงรายงานการเปลี่ยนแปลงนี้ในฐานะงานสืบเนื่อง มิใช่พฤติกรรมที่วัดได้] นอกจากนี้การวางเงินค้ำ GRX เพื่อรับรางวัลใช้ตัวสะสมแบบ MasterChef (acc_reward_per_share สเกลด้วย $10^12$) ที่เพิ่มขึ้นตามรางวัลที่เติมเข้า ($"delta" = "amount" times "ACC" slash "total_staked"$) และเมื่อมีการ slash หลักประกันจะถูกกระจายต่อให้ผู้วางเงินค้ำที่เหลือผ่านการเพิ่มตัวสะสมเดียวกัน

=== การประมวลผลแบบขนานด้วย Sealevel และการแบ่งชาร์ด <sec:sealevel-sharding>
รูปแบบการออกแบบบัญชีที่ปรากฏซ้ำในหลายโปรแกรมคือการใช้บัญชี PDA รายเอนทิตี เพื่อให้ธุรกรรมที่ไม่เกี่ยวข้องกันมีชุดบัญชีที่เขียนไม่ทับซ้อนและประมวลผลแบบขนานได้ตามแบบจำลอง Sealevel ของ Solana ตัวอย่างเช่น บัญชี MeterState รายมาตรวัดในโปรแกรม oracle, บัญชี Order และ order nullifier รายคำสั่งในโปรแกรม trading และบัญชี escrow รายผู้ใช้ สำหรับตัวนับรวมที่หากเขียนลงบัญชีเดียวจะกลายเป็นคอขวด (เช่น จำนวนผู้ใช้สะสม หรือยอดชำระสะสม) ระบบใช้การแบ่งชาร์ดจำนวน 16 ชาร์ด โดยในโปรแกรม registry (ผู้ใช้และมาตรวัด) และในสมุดคำสั่งของโปรแกรม trading (`market_shard` และ `zone_shard`) หมายเลขชาร์ดถูกอนุพันธ์แบบกำหนดได้จากไบต์แรกของกุญแจ (`key.to_bytes()[0] % 16`) ผู้เรียกจึงเลือกชาร์ดเองไม่ได้ ส่วนตัวสะสมยอดชำระในโปรแกรม treasury รับหมายเลขชาร์ดจากผู้เรียกและตรวจเพียงขอบเขตดังที่กล่าวข้างต้น แล้วจึงกระทบยอดเข้าตัวนับศูนย์กลางภายหลังด้วยคำสั่งระดับผู้ดูแล (aggregate_shards, aggregate_settlement_shards และ aggregate_readings) ที่ป้องกันการนับซ้ำด้วยบิตแมสก์ ผลคือบัญชีส่วนกลาง เช่น GovernanceConfig, OracleData และ Market ถูกออกแบบให้อ่านแบบล่าช้าได้ (stale) บนเส้นทางที่มีความถี่สูง โดยยอมรับว่าค่ารวมจะตามหลังเล็กน้อยเพื่อแลกกับ throughput ที่ขนานได้ การออกแบบนี้สอดคล้องกับเป้าหมาย slot time ใกล้ 400 มิลลิวินาทีที่อ้างถึงใน@sec:consortium-network ส่วนปริมาณงานธุรกรรมจริงบนเชนวัดบน validator เดี่ยวและรายงานใน@sec:onchain-throughput โดยตัวเลขบนเครือข่าย permissioned หลายโหนดที่รวมต้นทุน consensus ยังเป็นงานในอนาคต (ดู@sec:settlement-cost)
