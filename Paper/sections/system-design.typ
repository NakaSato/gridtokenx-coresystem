...
=== Treasury: การบันทึกการชำระและการตรึงค่า THBG <sec:treasury-program>
โปรแกรม treasury เป็นปลายทาง CPI ของการบันทึกการชำระจากโปรแกรม trading และเป็นแกนการเงินของระบบ การบันทึกการชำระแบบกลุ่ม (record_settlement_batch) เขียนบัญชี SettlementRecord รายคู่ (zone_id, batch_id) ที่เก็บรากเมอร์เคิล (merkle_root ขนาด 32 ไบต์) มูลค่ารวม และภาษีมูลค่าเพิ่ม (vat_amount, vat_rate_bps) เป็นข้อผูกพันสำหรับตรวจสอบย้อนหลัง โดยรากเมอร์เคิลผูกชุดธุรกรรมในกลุ่มไว้บนเชนขณะที่ใบไม้ (leaf) ของต้นไม้ถูกเก็บนอกเชน คำสั่งฐานนี้กระทบยอดรวมส่วนกลาง (total_settled_thbg) โดยตรง เพื่อรองรับการชำระพร้อมกันจำนวนมากโดยไม่ให้บัญชี treasury กลายเป็นคอขวด จึงมีคำสั่งแบบขนาน (record_settlement_batch_sharded) ที่บันทึก SettlementRecord เช่นเดิมแต่เพิ่มยอดลงบัญชีตัวสะสมรายชาร์ดแทนยอดรวมส่วนกลาง โดยแบ่งออกเป็น 16 ชาร์ด (`settle_shard_for(key) = key[0] % 16`) แล้วกระทบยอดเข้าศูนย์กลางภายหลังด้วย aggregate_settlement_shards ด้านการตรึงค่า stablecoin THBG คำสั่ง swap_grx_for_thbg บังคับว่าอุปทานใหม่ต้องไม่เกินทุนสำรองที่ได้รับการรับรอง (`new_supply <= attested_reserve`) และการรับรองต้องยังไม่หมดอายุ (`now - attestation_ts <= attestation_ttl`) ส่วน redeem_thbg_for_grx จำกัดการไถ่ถอนไม่ให้เกินหลักประกันในคลัง (`grx_out <= swap_vault.amount`) นอกจากนี้การวางเงินค้ำ GRX เพื่อรับรางวัลใช้ตัวสะสมแบบ MasterChef (acc_reward_per_share สเกลด้วย $10^12$) ที่เพิ่มขึ้นตามรางวัลที่เติมเข้า ($"delta" = "amount" times "ACC" slash "total_staked"$) และเมื่อมีการ slash หลักประกันจะถูกกระจายต่อให้ผู้วางเงินค้ำที่เหลือผ่านการเพิ่มตัวสะสมเดียวกัน
