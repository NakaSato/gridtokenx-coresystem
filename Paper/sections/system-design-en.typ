...
_A microservice realization: the Aggregator Bridge verifies Ed25519-signed readings, the CDA matches, and the Chain Bridge settles over a NATS JetStream queue — decoupling ingest, validation, and on-chain recording._
...
The voting uses weighting by the cumulative generation of meters (stake-by-generation), where a voter's weight is computed as $w = op("max")(100, "total generation" / 1000)$ — that is, every 1,000 kWh of cumulative generation is equivalent to one weight unit, with a minimum of 100 so that small participants still have a voice. Double-voting is prevented using a PDA vote record account per (proposal, voter) pair, one account per vote. When the voting period ends, execute_proposal tallies the result automatically: a proposal Passes only if the total votes reach the minimum quorum threshold (min_quorum_votes set in the GovernanceConfig account) and the votes in favor exceed the votes against; otherwise it is Rejected, thereby preventing parameter changes by too few voters.
    ...
    #diagram(
      ...
      node-stroke: 0.6pt,
      {
        ...
        edge((2, 0), (1, 0), "-|>", stroke: (paint: cp, dash: "dashed", thickness: 0.6pt),
          label: text(5pt, fill: cp)[read maintenance / REC])
...
#figure(
  ...
  caption: [Market-clearing timeline of the oracle program: signed meter readings arrive continuously over the 15-minute (900 s) aggregation window; at window close the oracle aggregates readings, triggers clearing, and the matched pair is settled on-chain. This is the on-chain telemetry path as designed and as benchmarked in isolation (@sec:meter-scale-en); the deployed pipeline does *not* write readings on-chain — it terminates ingest in the Redis streams and window bins of @fig:telemetry-dissemination and reaches the chain only at mint and settlement.],
) <fig:clearing-window>

=== Trading: On-chain Order Lifecycle and Escrow <sec:trading-program>
The trading program manages the lifecycle of trade orders and the on-chain escrow accounts, complementing the settlement path in @sec:settlement-model. Users create orders via create_sell_order and create_buy_order (as well as submit_limit_order and submit_market_order), which create a per-order PDA Order account bound to the owner. The order expiry (expires_at) is supplied by the client rather than stamped by the program: the program only requires the submitted value to lie strictly in the future, and reserves `0` as the no-expiry sentinel — the same meaning the settlement path reads from the signed payload (`expires_at == 0 || now < expires_at`), so no order can be live on one path and lapsed on the other. The value actually used is set by policy in the Trading Service off-chain layer, defaulting to 15 minutes (900 seconds, one market-clearing window) with a 7-day ceiling. A sell order must reference a REC certificate that has not yet expired before it can be created, consistent with the REC governance in @sec:governance-control-plane. Before entering the market, the user deposits tokens into the escrow account via deposit_escrow and withdraws the unused portion via withdraw_escrow, where the escrow account is an SPL token account under a PDA signed by the market_authority.

The program exposes two settlement entry points, and it matters which one a deployment uses. `settle_offchain_match` is the non-custodial form analyzed throughout @sec:settlement-model: each party's funds sit in that party's own escrow PDA, and every account the instruction touches is required to equal the canonical PDA derived from the payload the parties signed. `execute_atomic_settlement` is a custodial variant in which the escrow accounts are pooled associated-token accounts owned by the platform key (which is also the market authority), so a single platform signature satisfies both escrow-authority slots and the parties hold no escrow of their own. The deployment measured in this article routes its settlements through the custodial variant, because the prototype onboards users with platform-managed wallets rather than self-custodied keys. The consequence is stated plainly: for that path the guarantee that address derivation binds funds to the signing party (@sec:settlement-model) does not hold, and users must additionally trust the platform key not to disburse from the pool outside a matched settlement. Moving the deployment onto the non-custodial `settle_offchain_match` path — for which the per-party escrow PDAs and the funding instruction already exist — is the pay-down, and it is not a change to the settlement model but a change to which entry point the off-chain settlement service calls.
...
- Config gates: update_governance_config (enabling/disabling the REC check and transfers), update_erc_limits, set_maintenance_mode (the system-wide stop switch), update_authority_info, set_oracle_authority, and initialize_zone_config, which creates the per-zone parameter account. Every instruction must be signed by the authority.
...
- Fungible REC token system: init_rec_mint creates the REC mint once (1 token = 1 MWh, 6 decimals) and retire_rec burns tokens to retire certificates — distinct from the per-certificate ErcCertificate accounts of the preceding subsystem.
...
#figure(
  ...
  text(size: 8pt)[
    ...
    #table(
      ...
      [`GovernanceConfig`], [`[governance_config]`], [singleton: authority, pending_authority, REC policy, maintenance flag, min_quorum_votes, and counters],
...
One scoping point must be stated to avoid overstating what runs. In the deployed prototype the ingest path does not write readings on-chain: the Aggregator Bridge disseminates verified readings into the zone Redis Streams and the 15-minute window bins (@fig:telemetry-dissemination), and the forward to the on-chain oracle is a best-effort side channel whose unavailability must not block ingest. The chain is reached at the *settlement* boundary — the surplus mint at window close, and the trade settlement — not per reading. The on-chain ingest path described in this subsection is therefore implemented and measured in isolation as a scaling experiment (@sec:meter-scale-en), where it demonstrates that per-meter `MeterState` PDAs keep reading writes parallelizable to 200,000 meters, but it is not the path the end-to-end runs of @sec:evaluation exercise. Promoting it to the live pipeline would require a per-reading on-chain cost the design deliberately avoids paying.
