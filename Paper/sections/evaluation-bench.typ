...
เส้นทางการสร้างเหรียญจากพลังงานส่วนเกิน ซึ่งทดสอบแบบครบวงจรบน validator จริง (มิใช่ใน litesvm) ยืนยันภาพของ compute ที่บางเช่นเดียวกันจากปลายอีกด้านของ pipeline ธุรกรรม `mint_generation` สำหรับผู้รับรายเดียว (@sec:tx-structure) ที่วัดบนเชนใช้ราว 4,455 CU สำหรับการสร้างบัญชี associated token แบบ idempotent และราว 1,237 CU สำหรับการเรียกข้ามโปรแกรม (CPI) `MintTo` ของ Token-2022 ทำให้ทั้งธุรกรรมการสร้างเหรียญอยู่ต่ำกว่างบ 200,000 CU ต่อคำสั่งถึงหนึ่งลำดับขนาด และต่ำกว่าเส้นทางชำระที่ตรวจลายเซ็นมาก เช่นเดียวกับการชำระ การสร้างเหรียญหนึ่งครั้งมิได้ถูกจำกัดด้วยทั้ง compute และแพ็กเก็ต ต้นทุนของการขยายขนาดมาจากการรวมผู้รับหลายรายเท่านั้น ค่าธรรมเนียมธุรกรรม 10,000 lamports สอดคล้องกับลายเซ็นที่จำเป็นสองชุด คือ platform authority (ซึ่งเป็นผู้จ่ายค่าธรรมเนียมด้วย) และผู้ร่วมลงนาม REC validator ที่บังคับ อันเป็นการควบคุมแบบสองฝ่ายเดียวกับที่กำกับการสร้างเหรียญจากการผลิต
...
== การเปรียบเทียบกลไกราคาต่อรายได้ของผู้ผลิตร่วม <sec:price-mechanism-revenue>
การวิเคราะห์ใน@sec:revenue-sensitivity เป็นการคำนวณเชิงแบบจำลองจากพารามิเตอร์คงที่ที่ปริมาณจับคู่เดียว ส่วนนี้ยกระดับขึ้นเป็นการวัดบนภาระงานจำลองเต็มรูปแบบ โดยเปรียบเทียบรายได้สุทธิของผู้ผลิตร่วม (prosumer) จากพลังงานส่วนเกินภายใต้กลไกราคาสามแบบ ได้แก่ การประมูลราคาเดียว (uniform-price auction) ที่เคลียร์ทั้งสมุดคำสั่งที่ราคาเสนอขายส่วนเพิ่ม, CDA แบบจ่ายตามราคาเสนอ (pay-as-ask) และการรับซื้อคืนตามอัตราที่กำกับ (feed-in buy-back) ที่ 2.20 บาทต่อ kWh

ชุดข้อมูลสร้างจาก Smart Meter Simulator ด้วยแบบจำลองโครงข่ายเต็มรูปแบบ (load, solar, power flow, losses, tap control) เป็นห้าฟลีตขนาด 80, 160, 240, 320 และ 400 มาตรวัด โดยกำหนดสัดส่วนผู้ผลิตร่วมไว้ที่ 10% ของมาตรวัดทั้งหมดในทุกฟลีต ระยะเวลา 30 วัน ช่วงเวลาสุ่มตัวอย่าง 15 นาที (96 จุดต่อวัน) ด้วย seed เดียวกัน (42) จึงได้ลำดับสภาพอากาศชุดเดียวกันทุกฟลีต (แดดจัด 12 วัน, มีเมฆบางส่วน 8 วัน, ครึ้ม 4 วัน, เมฆมาก 5 วัน, ฝน 1 วัน) และกำหนดเพดานการจ่ายพลังงานเข้าโครงข่ายที่ 5 kWh ต่อผู้ผลิตร่วมต่อวัน เพดานนี้ผูกพัน (binding) เพียงบางวัน กล่าวคือพลังงานส่วนเกินที่ซื้อขายได้จริงอยู่ที่ 85.7–92.6% ของเพดาน ปริมาณส่วนเกินจึงถูกกำหนดโดยฟิสิกส์ของโครงข่าย มิใช่เอกลักษณ์เชิงเลขคณิตจากเพดาน

การคิดค่าธรรมเนียมใช้ตารางอัตราบนเชน (`TariffConfig`) ได้แก่ ค่าผ่านสาย (wheeling) แบบอัตราคงที่ 1.15 บาทต่อ kWh ค่าธรรมเนียมตลาด 25 bps และค่าการสูญเสีย 5 bps โดยการรับซื้อคืนได้รับยกเว้นค่าผ่านสายตามลักษณะของอัตราที่กำกับ ส่วนด้านอุปสงค์สร้างจากการบริโภคจริงของผู้ใช้ในฟลีตเดียวกัน (participation share $alpha = 0.02$ ในช่วงความเต็มใจจ่าย 2.60–4.10 บาทต่อ kWh) ผลสรุปไว้ใน@tbl:price-mechanism-revenue
...
#figure(
  ...
  text(size: 7pt)[
    ...
    #table(
      ...
      table.header([*มาตรวัด*], [*ผู้ผลิตร่วม*], [*ส่วนเกิน (kWh)*], [*uniform*], [*CDA*], [*buy-back*]),
...
จาก@tbl:price-mechanism-revenue มีข้อสังเกตสามประการ ประการแรกคือการสลับอันดับ (inversion) ระหว่างรายได้รวมก่อนหักและรายได้สุทธิ กล่าวคือ CDA สร้างมูลค่าซื้อขายรวม (gross) สูงกว่าการรับซื้อคืนราว 46% แต่ให้รายได้สุทธิแก่ผู้ผลิตร่วม#emph[ต่ำกว่า]ราว 6% ในทุกฟลีต สาเหตุคือค่าผ่านสายถูกคิดเป็นอัตราคงที่ต่อหน่วยพลังงาน ขณะที่ค่าธรรมเนียมตลาดและค่าการสูญเสียถูกคิดเป็นสัดส่วนของมูลค่า ค่าผ่านสายคงที่จึงมีลักษณะถดถอย (regressive) ต่อราคา คือหักเท่ากันทั้งจากหน่วยที่ขายได้ราคาสูงและต่ำ ราคาเสนอขายที่ทำให้ CDA เท่าทุนกับการรับซื้อคืนอยู่ที่ $(2.20 + 1.15) slash 0.997 approx 3.3601$ บาทต่อ kWh ซึ่งสูงกว่าสามในสี่ขั้นของบันไดราคาเสนอขายที่ใช้ (3.00, 3.15, 3.30 และ 3.45) การประมูลราคาเดียวจึงชนะเพราะจ่ายทุกขั้นที่ราคาส่วนเพิ่มขั้นสูงสุดเพียงราคาเดียว

ประการที่สองคืออัตราสุทธิต่อหน่วยไม่ขึ้นกับขนาดฟลีต ทั้งห้าฟลีตให้ค่าเท่ากันถึงทศนิยมตำแหน่งที่สี่ ได้แก่ uniform 2.2897, CDA 2.0653 และ buy-back 2.2000 บาทต่อ kWh เนื่องจากทั้งอุปทานและอุปสงค์ขยายตามสัดส่วนผู้ผลิตร่วมและผู้บริโภคที่ตรึงไว้ จุดเคลียร์ตลาดจึงไม่เปลี่ยน ผลนี้แสดงความคงเส้นคงวาของลำดับกลไกเมื่อขยายขนาด (scale-invariance of the mechanism ranking) มิใช่ผลของขนาดต่อราคา รายได้รวมจึงขยายแบบเชิงเส้นตามจำนวนผู้ผลิตร่วม

ประการที่สามคือผลลัพธ์นี้ได้รับการยืนยันบนเชนจริง มิใช่การคำนวณนอกเชนเพียงอย่างเดียว ผู้วิจัยรันสมุดคำสั่งของทั้งห้าฟลีตผ่านคำสั่ง `clear_auction` และ `match_orders` บน validator ที่ deploy โปรแกรมครบทุกตัว แล้วตรวจสอบซ้ำโดยอ่านสถานะบนเชนกลับมาเทียบ ผ่านทั้ง 5 ฟลีต รวม 35 การตรวจสอบ ครอบคลุมราคาเคลียร์ที่บันทึกใน `zone_market`, ผลต่างปริมาณสะสมในระดับหน่วยอะตอม, ราคาของ `TradeRecord` ทุกรายการเทียบกับราคาเสนอขายของตน และการคงอยู่ของการสลับอันดับข้างต้น

ข้อจำกัดของการวัดนี้มีสามประการที่ต้องระบุให้ชัด ประการแรกคือความหมายของระดับราคาในบันไดคำสั่งขาย ราคาเหล่านี้คือราคาที่ใช้ชำระจริง ซึ่งตามการก่อรูปราคาของเครื่องจับคู่คือราคาแบบ landed ที่บวกค่าผ่านสายไว้แล้วตาม@eq:clearing เมื่อการชำระบนเชนหักค่าผ่านสายออกจากราคานั้นอีกครั้ง ผู้ขายจึงได้รับยอดใกล้เคียงราคาเสนอขายตั้งต้นของตน ซึ่งสอดคล้องกับลักษณะต้นทุนแบบส่งผ่าน (pass-through) ที่วิเคราะห์ไว้ใน@sec:revenue-sensitivity มิได้ขัดแย้งกัน ตัวเลขทั้งสามคอลัมน์ใน@tbl:price-mechanism-revenue จึงเป็นยอดฝั่งผู้ขายที่เทียบกันได้ อย่างไรก็ตาม การจัดอันดับระหว่าง CDA กับการรับซื้อคืนขึ้นกับ#emph[ระดับ]ของบันไดราคาที่กำหนดไว้ ซึ่งเป็นค่าภายนอกแบบจำลอง (exogenous) กล่าวคือบันไดที่ 3.00–3.45 บาทต่อ kWh เทียบเท่าราคาเสนอขายฝั่งผู้ขายราว 1.85–2.30 บาทต่อ kWh ซึ่งคร่อมอัตรารับซื้อคืน 2.20 พอดี หากบันไดราคาอยู่สูงกว่าจุดเท่าทุน 3.3601 การจัดอันดับดังกล่าวจะกลับด้าน ผลที่ไม่ขึ้นกับระดับบันไดราคาคือการที่การประมูลราคาเดียวให้ยอดสูงกว่า CDA เสมอ ซึ่งเป็นผลของค่าผ่านสายแบบคงที่ต่อหน่วยตามที่อธิบายข้างต้น ประการที่สอง อุปสงค์ที่สร้างขึ้นมีปริมาณมากกว่าอุปทาน 2.30–2.42 เท่า สมุดคำสั่งจึงเคลียร์เต็มจำนวนที่ราคาเสนอขายสูงสุดเสมอ ซึ่งเป็นผลลัพธ์ที่เอื้อต่อการประมูลราคาเดียวมากที่สุด หากอุปสงค์ขาดแคลน ราคาเคลียร์จะต่ำลงและลำดับอาจเปลี่ยน และประการที่สาม ค่า $alpha$ และช่วงความเต็มใจจ่ายเป็นสมมติฐานเชิงรูปแบบ (stylised) มิใช่ค่าที่วัดจากพฤติกรรมผู้ใช้จริง การวัดสวัสดิการเต็มรูปแบบและพฤติกรรมเชิงกลยุทธ์ของผู้เข้าร่วมยังคงเป็นงานในอนาคต (ดู@sec:discussion_limitations)
