...
โครงสร้าง escrow ที่อธิบายข้างต้น คือหนึ่งบัญชี escrow PDA ต่อหนึ่งฝ่ายและผูกที่อยู่บัญชีเข้ากับ payload ที่ลงนาม เป็นแบบจำลองที่หัวข้อนี้วิเคราะห์และเป็นแบบที่คำสั่ง `settle_offchain_match` กับ `batch_settle_offchain_match` ใช้งาน ทั้งนี้โปรแกรมยังมีจุดเข้าการชำระอีกทางหนึ่งแบบ custodial (`execute_atomic_settlement`) ซึ่งบัญชี escrow เป็นบัญชีโทเคนแบบรวมกองที่เป็นของกุญแจแพลตฟอร์มแทนที่จะเป็นของคู่สัญญา และเป็นเส้นทางที่ต้นแบบซึ่งติดตั้งใช้งานเรียกอยู่ในปัจจุบัน (ดู@sec:trading-program) สมบัติคงที่ (invariant) ทุกข้อที่กล่าวต่อจากนี้เป็นของเส้นทาง non-custodial ส่วนบนเส้นทาง custodial นั้น สมบัติที่ได้มาจากการ#strong[ผูกที่อยู่บัญชี] กล่าวคือผู้โจมตีไม่อาจสลับบัญชี escrow ของเหยื่อเข้ามา และอำนาจจ่ายเงินออกเป็นสมบัติของเส้นทางโค้ดมิใช่ของกุญแจที่ผู้ดูแลถือไว้ ย่อมถูกแทนที่ด้วยความเชื่อถือในกุญแจแพลตฟอร์ม ขณะที่สมบัติที่บังคับจาก *payload* ได้แก่ ลายเซ็นทั้งสองฝ่าย order nullifier การครอสราคา เพดานการไหล และการแบ่งมูลค่าด้วยเลขคณิตแบบตรวจสอบ ยังคงเป็นจริงเช่นเดิม
...
เส้นทางการสร้างเหรียญจากพลังงานส่วนเกิน (surplus generation-mint) อยู่ที่ปลายตรงข้ามของสเปกตรัมนี้ เมื่อหน้าต่างการชำระปิดลงโดยมีการผลิตสุทธิเป็นบวก Aggregator Bridge จะสร้างเหรียญส่วนเกินผ่านคำสั่ง `mint_generation` ของโปรแกรม energy-token ซึ่งมีข้อมูลคำสั่งขนาดคงที่ 40 ไบต์ (discriminator 8 ไบต์, ตัวระบุมาตรวัด 16 ไบต์, เวลาเริ่มหน้าต่างแบบมิลลิวินาที 8 ไบต์ และจำนวนเหรียญ 8 ไบต์) และไม่มี payload ลายเซ็นฝังในคำสั่งเลย เพราะที่มา (provenance) ถูกยืนยันด้วยผู้ร่วมลงนาม REC validator ที่ลงทะเบียนแล้วในระดับลายเซ็นธุรกรรม ต่างจากการชำระที่ต้องฝังข้อความ Ed25519 ไว้ในข้อมูลคำสั่ง ธุรกรรมที่ Chain Bridge ส่งประกอบด้วยสามคำสั่งระดับบน (compute-budget, การสร้างบัญชี associated token แบบ idempotent และ `mint_generation`) รวมขนาดราว 700–750 ไบต์บนสาย (ลายเซ็น 64 ไบต์สองชุด, header กับ blockhash ราว 80 ไบต์, คีย์บัญชี 32 ไบต์ราวสิบสามรายการ และข้อมูลคำสั่งราว 50 ไบต์) ซึ่งอยู่ต่ำกว่าขนาดแพ็กเก็ต 1,232 ไบต์มาก การสร้างเหรียญให้ผู้รับรายเดียวจึงมิได้ถูกจำกัดด้วยทั้งแพ็กเก็ตและ compute (@sec:cu-profile) และมีพฤติกรรมการขยายขนาดตรงข้ามกับการชำระ กล่าวคือสามารถรวมผู้รับหลายรายไว้ในธุรกรรมเดียว (`build_generation_mint_instructions`) และเข้าใกล้เพดานแพ็กเก็ตเมื่อจำนวนผู้รับเพิ่มขึ้น มิใช่ถูกจำกัดที่หนึ่งหน่วยงานต่อธุรกรรม ทั้งนี้สิทธิ์อนุญาตของการสร้างเหรียญมิได้อยู่ในธุรกรรม แต่อยู่ในคำขอผ่าน NATS ที่นำหน้า โดย aggregator ลงนามบนการ serialize แบบ canonical ที่ติด domain tag และนำหน้าความยาวแต่ละฟิลด์ (correlation/idempotency key, กระเป๋าผู้รับ, ตัวระบุมาตรวัด, หน้าต่าง และปริมาณพลังงาน) ด้วยคีย์ P-256 ที่ Chain Bridge ผูกใบรับรองเข้ากับอัตลักษณ์บริการ (SPIFFE) ของผู้ขอ จึงปฏิเสธอัตลักษณ์ปลอมก่อนสร้างธุรกรรมใด ๆ และการสร้างเหรียญแบบครั้งเดียวพอดี (exactly-once) ถูกบังคับบนเชนด้วยบัญชีบันทึกต่อคู่ `(มาตรวัด, หน้าต่าง)` ซึ่งมีธง `minted` ที่ทำให้การส่งซ้ำกลายเป็น no-op ดังนั้นการชำระที่ลองใหม่หลังระบบล่มจึงไม่อาจสร้างเหรียญซ้ำได้

== ซองข้อความ mint ที่ลงนามและการส่งมอบแบบไม่สูญหาย <sec:signed-mint-envelope>

สิทธิ์อนุญาต (authorization) ของการสร้างเหรียญส่วนเกินไม่ได้ฝังอยู่ในธุรกรรมบนเชนดังเช่นการชำระ แต่ถูกยกออกไปไว้ในซองข้อความ (envelope) ที่ Aggregator Bridge ส่งให้ Chain Bridge ผ่าน NATS บนหัวข้อ `chain.tx.mint` แนวคิดคือแยกการพิสูจน์ที่มา (provenance) ออกจากการสร้างธุรกรรม กล่าวคือ Chain Bridge จะยอมประกอบและลงนามธุรกรรม `mint_generation` ด้วยคีย์ `platform_admin` ก็ต่อเมื่อซองข้อความผ่านการตรวจอัตลักษณ์ระดับแอปพลิเคชันแล้วเท่านั้น การลงนามบนซองจึงเป็นชั้นควบคุมสิทธิ์ที่ทำงานก่อนบล็อกเชน มิใช่ส่วนหนึ่งของ payload ที่โปรแกรมบนเชนตรวจ

การลงนามกระทำบนการเข้ารหัสแบบ canonical ที่ออกแบบให้ไม่กำกวมและกันการเล่นซ้ำ (replay) โครงร่างประกอบด้วยแท็กโดเมน (domain tag) `gridtokenx-nats-envelope-v1` ที่ปิดท้ายด้วยไบต์ศูนย์ ตามด้วยแท็กชนิดข้อความ (`mint`) เพื่อแยกโดเมนของแต่ละชนิด (signed cancel เล่นซ้ำเป็น submit ไม่ได้) จากนั้นแต่ละฟิลด์ถูกเข้ารหัสเป็น ชื่อฟิลด์ + ไบต์ศูนย์ + ความยาวแบบ `u64` little-endian + ค่าไบต์ดิบ การนำหน้าความยาวต่อฟิลด์ทำให้ไม่สามารถเลื่อนขอบเขตไบต์ข้ามฟิลด์เพื่อเล่นซ้ำลายเซ็นที่จับไว้ได้ ลำดับฟิลด์ถูกกำหนดตายตัว (มิใช่ลำดับตาม JSON) และฟิลด์ `auth` ที่บรรจุลายเซ็นถูกกันออกจากไบต์ที่ลงนามเสมอเพราะลายเซ็นครอบตัวเองไม่ได้ โครงสร้างฟิลด์ทั้งหมดของซอง `mint` แสดงใน@tbl:mint-envelope-bytes โดยฟังก์ชันเข้ารหัสถอดโครงสร้าง (destructure) ทุกฟิลด์อย่างครบถ้วนโดยเจตนา หากมีการเพิ่มฟิลด์ใหม่ในสคีมาแต่ยังไม่ระบุว่าให้ลงนามหรือยกเว้น โค้ดจะคอมไพล์ไม่ผ่าน ป้องกันฟิลด์ที่เดินทางบนสายแต่หลุดจากลายเซ็นโดยไม่ตั้งใจ

#figure(
  caption: [โครงร่างไบต์ที่ลงนามแบบ canonical ของซองข้อความ mint บนหัวข้อ NATS `chain.tx.mint` (message kind `mint`) แต่ละแถวคือหนึ่งฟิลด์ที่เข้ารหัสเป็น `ชื่อ + 0x00 + ความยาว u64-LE + ค่า` เรียงตามลำดับที่ลงนามจริง ส่วนหัวนำด้วย domain tag `gridtokenx-nats-envelope-v1\\0mint\\0` และฟิลด์ `auth` ถูกกันออกจากไบต์ที่ลงนาม.],
  text(size: 7pt)[
    ...
    #table(
      ...
      table.header([ฟิลด์], [การเข้ารหัสค่า], [ความหมาย]),
      [`correlation_id`], [UTF-8], [UUID ต่อครั้งที่ส่ง; ใช้กำหนดหัวข้อรับผล],
      [`idempotency_key`], [UTF-8], [`mint:{serial}:{window}` คีย์กันซ้ำระดับผล],
      [`reply_subject`], [UTF-8], [หัวข้อ NATS ที่ผู้ส่งรอรับผล],
      [`recipient_wallet`], [UTF-8], [กระเป๋าผู้รับ (base58)],
      ...
      [`meter_id`], [16 ไบต์ดิบ], [ตัวระบุมาตรวัด (UUID)],
      [`window_start_ms`], [`i64` LE (8B)], [เวลาเริ่มหน้าต่าง 15 นาที],
    )
  ],
) <tbl:mint-envelope-bytes>

ไบต์ canonical ข้างต้นเป็นเพียง อินพุตของลายเซ็น เท่านั้น สิ่งที่เดินทางบนสาย NATS จริงคือซอง JSON (`MintEnergyMessage`) ที่บรรจุฟิลด์เหล่านั้นพร้อมฟิลด์ `auth` แนบท้าย โดย `auth` (`EnvelopeAuth`) ประกอบด้วยแท็กสคีมา `ecdsa-p256-sha256-v1` ใบรับรอง mTLS ปลายทาง (leaf cert PEM) และลายเซ็น ECDSA P-256/SHA-256 ที่เข้ารหัส ASN.1-DER แล้ว base64 ผู้ลงนามคือ aggregator ที่ถือคีย์ไคลเอนต์ mTLS เดียวกับที่ใช้เป็นขอบเขตความเชื่อถือของ gRPC เมื่อไม่มีคู่ใบรับรอง/คีย์ (โหมด dev แบบไม่ปลอดภัย) ซองจะถูกส่งแบบไม่ลงนามและถูกยอมรับเฉพาะขณะที่ Chain Bridge เดินในโหมดบันทึกอย่างเดียว (log-only)

ฝั่ง Chain Bridge ตรวจสามชั้นตามลำดับก่อนสร้างธุรกรรม คือ (1) ใบรับรองในซองต่อสายไปยัง CA ที่เชื่อถือ (2) ค่า SPIFFE SAN ในใบรับรองต้องเท่ากับฟิลด์ `service_identity` ที่ซองประกาศ ผูกคำกล่าวอ้างอัตลักษณ์ที่ประกาศเองเข้ากับใบรับรองที่ CA ออก และ (3) ตรวจลายเซ็น P-256 เทียบกับไบต์ canonical ที่ คำนวณใหม่ จากฟิลด์ที่รับมา ดังนั้นการดัดแปลงฟิลด์ใด ๆ ระหว่างทาง (เช่น เปลี่ยนกระเป๋าผู้รับหรือปริมาณพลังงาน) จะทำให้ไบต์ที่คำนวณใหม่ต่างออกไปและลายเซ็นตรวจไม่ผ่าน การบังคับตรวจลายเซ็นเปิดใช้ด้วยธง `CHAIN_BRIDGE_REQUIRE_SIGNED_NATS` มิฉะนั้นทำงานแบบ log-only เพื่อการย้ายระบบแบบค่อยเป็นค่อยไป จุดสำคัญเชิงวิศวกรรมคือฝั่งผู้ลงนามและฝั่งผู้ตรวจใช้การเข้ารหัส canonical ชุดเดียวกันจากไครเอตที่แบ่งใช้ร่วม (`gridtokenx-blockchain-types`) มิใช่การคัดลอกโครงไบต์แยกกัน จึงมีการทดสอบ golden-vector ที่ประกอบไบต์ที่คาดหวังขึ้นใหม่โดยอิสระเพื่อจับความคลาดเคลื่อนของสคีมาข้ามไครเอต

การส่งมอบซองออกแบบให้ไม่สูญหายแม้เมื่อบล็อกเชนหรือ validator ล่มชั่วคราว ซองถูก publish เข้าสู่ JetStream และรอรับ PubAck (การเก็บลงสตรีมแบบทน) ก่อนรอผลลัพธ์ ปิดช่องที่ผู้บริโภคอาจหายไปชั่วขณะ นอกจากนี้ส่วนเกินที่ชำระออกจากหน้าต่างแล้วแต่ยังลงเชนไม่ได้จะถูก เข้าคิว (enqueue) ลง durable outbox บน Redis (`gridtokenx:billing:mint_outbox` ฟิลด์ `{serial}:{window}`) และมี drain loop คอยลองซ้ำจนกว่าการสร้างเหรียญจะได้รับการยืนยันบนเชนจึงลบออก คิวนี้อยู่รอดการรีสตาร์ต (โหลดกลับจาก Redis) และกระเป๋าผู้รับถูก resolve ใหม่ทุกครั้งที่ลอง ทำให้มาตรวัดที่ลงทะเบียนกระเป๋า ภายหลัง หน้าต่างของตนยังได้รับเหรียญในการลองครั้งถัดไป การลองซ้ำปลอดภัยเพราะ Chain Bridge กันซ้ำด้วย `mint:{serial}:{window_start_ms}` ประกอบกับบัญชี PDA `(มาตรวัด, หน้าต่าง)` บนเชนเป็นด่านสุดท้าย การลองซ้ำของเหรียญที่ลงเชนไปแล้วแต่ผลตอบกลับหาย จึงไม่สร้างเหรียญซ้ำ รายการที่ยังลงเชนไม่ได้เกินกำหนดเวลา (`MINT_OUTBOX_MAX_AGE_SECS` ค่าเริ่มต้น 7 วัน) จะถูก พัก (park) ออกจากเส้นทางลองซ้ำแทนการลบทิ้งเงียบ ๆ เพื่อให้ผู้ดูแลนำกลับเข้าคิวได้ ส่วนคำขอแบบกลุ่ม (batch) บนหัวข้อ `chain.tx.mintbatch` ใช้โครงร่างเดียวกันแต่ผนวกฟิลด์จำนวนรายการ (`item_count`) และห่อแต่ละรายการด้วยกรอบความยาวของตนเอง จึงผูกทั้ง จำนวน และ ลำดับ ผู้รับเข้ากับลายเซ็น การเพิ่ม ลด หรือสลับลำดับผู้รับจะทำให้ลายเซ็นตรวจไม่ผ่าน

มีฟิลด์หนึ่งของซองที่ข้ามขอบเขตการแทนค่า (representation) ซึ่งควรระบุให้ชัด เพราะเป็นจุดที่แยกค่าอ่านที่สุจริตออกจากค่าที่ประสงค์ร้ายก่อนสร้างเหรียญ ปริมาณพลังงานเดินทางเป็นเลขทศนิยม `f64` หน่วย kWh แต่การสร้างเหรียญบนเชนทำงานด้วยจำนวนเต็มหน่วยฐาน (base unit) ที่ 9 ตำแหน่งทศนิยม (1 kWh = $10^9$ หน่วยฐาน ตามความละเอียดของ energy token) การแปลงค่าใช้ฟังก์ชันร่วมเพียงตัวเดียวทั้งฝั่งผู้ผลิต (aggregator) และฝั่งผู้ตรวจ (Chain Bridge) ตามหลักแหล่งเดียว (single source) เช่นเดียวกับการเข้ารหัส canonical จึงไม่มีทางที่ทั้งสองฝั่งจะเห็นขอบเขตหรือการสเกลต่างกัน ฟังก์ชันนี้ปฏิเสธค่าที่ไม่ควรถึง `mint_to` ได้แก่ ค่าที่ไม่จำกัด (`NaN`/`±∞`) ค่าที่ไม่เป็นบวก ค่าที่เกินเพดาน `MAX_MINT_KWH` ที่ $10^6$ kWh (1 GWh สูงกว่าหน้าต่าง 15 นาทีที่สุจริตมากแต่ยังต่ำกว่าจุดล้น) และค่าบวกที่ตัดเศษแล้วเหลือศูนย์หน่วยฐาน เพดานนี้เป็นกลไกสำคัญมิใช่เพียงประดับ เพราะการแปลงเลขทศนิยมเป็นจำนวนเต็มในภาษา Rust จะ อิ่มตัว (saturate) เป็น `u64::MAX` เมื่อค่ามหาศาลแทนการวนกลับ (wrap) หากไม่มีเพดานนี้ซองเดียวอาจร้องขอการสร้างเหรียญมหาศาลได้ เพดานจึงบีบค่าที่สเกลแล้วให้อยู่ราว $10^{15}$ ซึ่งอยู่ในช่วง `u64` อย่างปลอดภัย จากนั้นโปรแกรมบนเชนตรวจซ้ำอีกชั้นเป็นการป้องกันเชิงลึก (defense in depth) โดยปฏิเสธจำนวนศูนย์ (`ZeroAmount` รหัสข้อผิดพลาด `6011`) ก่อนการเขียนสถานะใด ๆ เพราะ `mint_to(_, 0)` เป็น no-op เงียบ ๆ ที่ยังคงตีธง `minted = true` ให้หน้าต่างนั้นและทำให้หน้าต่างเสียถาวร ร่วมกับการตรวจขอบหน้าต่าง 15 นาทีในหน่วยมิลลิวินาที (`MisalignedWindow` รหัส `6010`) และเงื่อนไขที่ผู้ร่วมลงนาม REC validator ต้องต่างจากผู้มีอำนาจแพลตฟอร์ม (`RecValidatorIsAuthority` รหัส `6012`) ทั้งนี้บัญชีบันทึกแบบครั้งเดียวพอดีถูกตรวจ ก่อน เสมอ การส่งซ้ำจึงลัดเป็น no-op ก่อนที่ด่านเหล่านี้จะทำงาน
