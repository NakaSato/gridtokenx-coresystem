...
ก็ต่อเมื่อผลรวมคะแนนถึงเกณฑ์องค์ประชุมขั้นต่ำ (`min_quorum_votes` ที่กำหนดใน `GovernanceConfig`)
...
ที่ตรึงค่ากับเงินบาท โดยใช้อัตรา $r$ (หน่วยย่อย THBG ต่อ 1 GRX) และค่าธรรมเนียม swap $psi$ (bps)
...
(self-trade) และรองรับคำสั่งแบบ Fill-or-Kill โดยเครื่องจับคู่ปล่อยผลการจับคู่แต่ละรายการออกโดยตรง
ไม่มีขั้นรวมรายการ (no consolidation step) เนื่องจากคำสั่งขายแต่ละรายการอยู่ในสมุดของโซนเดียว
คู่ผู้ซื้อ-ผู้ขายหนึ่งคู่จึงให้ผลลัพธ์ไม่เกินหนึ่งรายการต่อรอบจับคู่
...
`create_sell_order`, `create_buy_order` และ `submit_limit_order`
ซึ่งสร้างบัญชี Order แบบ PDA รายคำสั่งที่ผูกกับเจ้าของ (seed `[order, authority, order_id]`)
...
แต่ปล่อยเหตุการณ์ `MarketOrderSubmitted` ให้จับคู่ในชั้น off-chain แทน
ทั้งนี้ใบรับรอง ERC เป็นตัวเลือกเสริมสำหรับคำสั่งขาย (optional)
เมื่อแนบมาต้องมีสถานะ Valid ยังไม่หมดอายุ และผ่าน `validate_erc_for_trading` แล้ว ก่อนเข้าสู่ตลาด
...
ออกแบบเป็นโปรแกรมเดียวที่รวม 22 instruction โดยมีห้าระบบย่อยหลักตามหน้าที่
(เสริมด้วยคำสั่งเฉพาะ เช่น การเริ่มต้น REC mint การตั้งค่า `ZoneConfig` และ `get_governance_stats`)
...
#align(center)[
  #text(size: 11pt)[
    #table(
      ...
      [`GovernanceConfig`], [`[governance_config]`], [singleton: authority, pending_authority, นโยบาย ERC, maintenance flag, min_quorum_votes และตัวนับ],
...
(สูงสุด 5 ราย) เส้นทาง `mint_to_wallet` และ `mint_generation`
บังคับการร่วมลงนามของผู้รับรอง REC เสมอ (ปฏิเสธเมื่อยังไม่ตั้งผู้รับรอง)
โดย `mint_generation` บังคับเพิ่มเป็นแบบ 2-of-2 คือผู้ร่วมลงนาม REC ต้องไม่ใช่ authority เดียวกัน
ส่วน `mint_tokens_direct` บังคับการร่วมลงนามเฉพาะเมื่อมีการตั้งผู้รับรองไว้แล้วเท่านั้น
...
ธงยังเป็นเท็จทำให้ลองใหม่ได้ ส่วนคำสั่ง `retire_energy_tokens` เผาโทเคนผ่าน token interface ของ SPL Token-2022 [33]
...
(`shard_id` ที่ผู้เรียกส่งเข้ามาและตรวจช่วง `shard_id < 16`) แล้วกระทบยอดเข้าศูนย์กลางภายหลังด้วย
...
ระบบใช้การแบ่งชาร์ดจำนวน 16 ชาร์ด โดยโปรแกรม registry เลือกชาร์ดแบบกำหนดได้จากไบต์แรกของกุญแจ
(`key.to_bytes()[0] % 16`) ส่วนโปรแกรม treasury รับ `shard_id` จากผู้เรียกและตรวจช่วง (`< 16`)
แล้วจึงกระทบยอดเข้าตัวนับศูนย์กลางภายหลังด้วยคำสั่งระดับผู้ดูแล
...
กล่าวคือการรวมสองคู่ (คำสั่งตรวจลายเซ็น 4 คำสั่ง ร่วมกับ BatchMatchPair ที่ serialize แล้ว
และรายการดัชนีบัญชีกับ header) ก็เกินขนาดแพ็กเก็ตแล้ว (ค่าไบต์ต่อคำสั่งตรวจลายเซ็นในที่นี้เป็นการประเมินจากโครงสร้างคำสั่ง Ed25519 มิใช่ค่าที่วัดในโค้ด)
