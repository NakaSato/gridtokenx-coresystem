...
if [ "${E2E_RUN_OPENADR:-0}" != "1" ]; then
    log_skip "OpenADR suite skipped (set E2E_RUN_OPENADR=1 — slow: cargo build + VTN stack, stops the bridge container)"
    suite_summary; exit 0
fi

if [ ! -f "$SCRIPT" ]; then
    log_skip "scripts/openleadr-e2e.sh not found — skipping"
    suite_summary; exit 0
fi

for bin in docker cargo python3 curl; do
    if ! command -v "$bin" >/dev/null; then
        log_skip "$bin not on PATH — skipping (OpenADR test toolchain required)"
        suite_summary; exit 0
    fi
done
...
if ! docker info >/dev/null 2>&1; then
    log_skip "docker daemon unreachable — skipping OpenADR suite"
    suite_summary; exit 0
fi
