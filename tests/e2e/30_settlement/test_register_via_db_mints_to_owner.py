...
only; register via meter-service API → meter database".
  ...
  (NO gridtokenx:meters:{serial}:* in Redis)
  signed backdated surplus telemetry -> :4030 ingest
    -> Ed25519 verify (device pubkey in Redis)
...
def test_db_registered_meter_mints_to_owner_without_redis_seed():
    ...
    serial = f"E2E-DBTIER-{int(time.time() * 1000) % 1_000_000}"
    priv, pub_hex = crypto.new_identity()
    gen = 50  # surplus: net = 50 - 0

    _register_meter_via_api(token, serial)

    # Device pubkey is needed for Ed25519 verification (separate from ownership).
    ...
    def _trigger():
        _ingest(_payload(serial, priv, kwh=gen, generated=gen, consumed=0, ts_ms=ts_ms))
