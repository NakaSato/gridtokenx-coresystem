...
# own tests/invariants.rs. We wrap it so it runs as part of the e2e gate.
...
if [ ! -d "$CB" ] || [ ! -f "$CB/tests/invariants.rs" ]; then
...
if command -v cargo >/dev/null; then
    ...
    if ( cd "$CB" && env -u CHAIN_BRIDGE_INSECURE cargo test --test invariants -- --nocapture ) ; then
