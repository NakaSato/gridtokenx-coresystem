...
> **[BUILT 2026-06-07] REAL-FLOW generation-mint, driven directly at trading (NO :4000).** New test `30_settlement/test_real_generation_mint.py` exercises the genuine settlement→on-chain mint path by POSTing the gen-mint endpoint that lives **inside trading-service** — the exact endpoint the oracle `SettlementEngine` targets via `SETTLEMENT_API_URL` (default `http://trading-service:8093`, fallback `TRADING_HTTP_URL`). Removes the out-of-repo `:4000` dependency. The test signs the batch request itself with the oracle-bridge signing key (impersonating the oracle's settlement signer, the legitimate caller).
> - **Endpoint:** `POST :8093/api/v1/settlement/generation-mint/batch` `{"requests":[{user_id, meter_serial, energy_generated_kwh(str), start_time(i64), end_time(i64), signature(base58)}]}`. **No gateway-secret/role layer** on settlement routes (only `TraceLayer` — `startup.rs:175-186`); sig verification is the sole gate. Canonical signed msg = `"{user_id}:{meter_serial}:{energy_generated_kwh}:{start}:{end}"` verified vs `ORACLE_BRIDGE_PUBLIC_KEY` (`rest.rs:914`). kWh must be a decimal **string** matching Rust `Decimal::Display` (use integer e.g. `"50"`).
...
> - **Bring-up:** trading with `AUTHORITY_WALLET_PATH=<dev-wallet.json>` (mint authority `EzudwoHv…`) + `ORACLE_BRIDGE_PUBLIC_KEY=<test signing pubkey>`; test holds the matching key via `ORACLE_BRIDGE_SIGNING_KEY` (keypair json path or base58 seed). py_compile + helper smoke (PDA/sig/ATA) PASS; full live mint **not yet run** (needs stack with the authority wallet + matching oracle pubkey).
