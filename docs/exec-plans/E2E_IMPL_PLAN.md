...
- [x] Create tree:
  ...
  ```
- [x] `env.sh` — centralize ports/secrets from README port table (`IAM=4010`, `TRADING=8093/8092`, `ORACLE=4030`, `CHAIN=5040`, `NOTI=5050`, `APISIX=4001`, `API=4000`, `PG=7001`, `REDIS=7010`, `KAFKA=29001`). *(`ENVOY=4002` removed 2026-06-14 with the edge.)*
...
- [~] Add: login happy/wrong-pass ✓, RBAC 403 (auth±) ✓, idempotent register 409 ✓ — **JWT refresh+rotation dropped** (no `/refresh` route in IAM *at the time*; a `/refresh` route now exists — `startup.rs:120`, `POST /api/v1/auth/refresh`).
...
- [x] 15-min aggregation window correctness — **DONE 2026-06-14** (`30_settlement/test_aggregation_window.py`). The in-crate unit tests cover the floor math; this proves the LIVE service does it end-to-end, observed via the durable bin store (`gridtokenx:settlement:bins` — write-through in `zone_ingester` → `bin_store.persist`, value a JSON `BillingBin`). Ingests two signed readings in the **same** still-open window + one in a **future** window, then asserts: (a) the two merge into ONE bin (`reading_count==2`, energy summed: gen 5.0 / cons 1.5 kWh); (b) `start_time` floors to the quarter hour (`minute % 15 == 0`, secs/nanos zeroed) and `end_time == start + 15 min`; (c) the future-window reading is a SEPARATE bin (distinct `start_time`, `reading_count==1`). Uses the current/future windows (end_time > now) + `SETTLEMENT_GRACE_SECS=120` cushion so nothing settles mid-test; REST has no replay/freshness gate (line 58) so a future-dated reading floors normally. New `redis_util.settlement_bins`/`delete_settlement_bins` read+clean the hash. Live PASS (5s). _(The harness sweep also added `X-API-KEY` to the last two non-pytest ingest tools — `test_telemetry_security.py`, `manual_path_b.py` — completing the `api_key_auth` migration.)_ **SUPERSEDED 2026-06-15:** the `gridtokenx:settlement:bins` write-through was removed from the codebase — aggregation/settlement migrated to `meter-service` (aggregator forwards readings via NATS `meter.reading`). `test_aggregation_window.py` + the `redis_util.settlement_bins`/`delete_settlement_bins` helpers were deleted (asserted a mechanism that no longer exists); the NATS→Chain-Bridge mint path is covered by `test_path_b_generation_mint.py` / `test_settlement_idempotency.py`.
...
- [—] Envoy mTLS enforcement: non-mTLS client at `:4002` rejected — **DONE 2026-06-13, then REVERTED/REMOVED 2026-06-14.** The Envoy `:4002` edge (mTLS listener + `aggregator_iot` routing) was deleted entirely: `envoy` service dropped from `docker-compose.yml`, `envoy_conf/` deleted, `80_gateways/test_envoy_mtls.py` removed, scripts/env scrubbed. IoT devices now ingress **directly** to the Aggregator Bridge IoT gateway with no transport-mTLS edge; device auth is Ed25519-only at the Aggregator. TD-003 reopened. *(Historical: the 2026-06-13 work rewrote `envoy_conf/envoy.yaml` as an mTLS listener that proxied a CA-signed client to `aggregator-bridge:4010`; that config no longer exists.)*
- [x] Simulator integration: the simulator's real OBIS-coded REST frame round-trips to dissemination — **DONE 2026-06-14**. (Premise corrected: there is **no InfluxDB** in the aggregator path — that earlier wording was wrong; the simulator's [`Dockerfile` `INFLUX_*`](../../gridtokenx-smartmeter-simulator/Dockerfile) is the standalone sim's own optional run-plot bucket, off by default. Path A sink is **Redis Streams + Kafka**, per superproject `CLAUDE.md`.) `20_oracle/test_simulator_obis_contract.py` mirrors the producer's payload (`gridtokenx-smartmeter-simulator/.../transport/aggregator_bridge.py::_build_obis_payload`) — OBIS active import/export in **Wh** (`1.1.1.8.0.255`/`1.1.2.8.0.255`) + convenience `kwh`/`energy_*` — POSTs `protocol="dlms"` to `/v1/private-network/ingest`, and asserts the disseminated `DeviceReading` decodes **OBIS Wh→kWh** (export 5000 Wh → `generated_kwh==5.0`) with **OBIS precedence over a decoy plain `energy_generated`=999** (`dlms.rs map_payload` OBIS-wins resolution, not HashMap last-write). Added `redis_util.find_disseminated_reading` to read the zone-stream payload (value-level assert, not just stream growth). **Also fixed a latent suite regression found here:** `api_key_auth` now gates ingest (`auth.rs`) and the older REST-ingest tests (`test_telemetry.py`, `30_settlement/test_settlement.py`, `90_golden_path`) posted **without** `X-API-KEY` → 401 at the auth layer; all now send the harness key (`AGGREGATOR_API_KEY`, default `e2e-test-key`, seeded in `docker-compose.yml:755` `GRIDTOKENX_API_KEYS`). 20_oracle + settlement + golden_path: 14P/7skip green. (`just send-meter-reading`/`auto-meter-send` live-fleet smoke is a separate manual tool — not a pytest gate.)
...
> **Deferred:** 15-min aggregation emission (→ Phase 4 settlement), replay (gRPC UTT-H, service.rs:166). _(Envoy mTLS enforcement — removed 2026-06-14 with the edge; see above.)_
...
- [x] Async submit: publish `chain.tx.submit` (NATS), assert signed + landed — **DONE 2026-06-14** (`50_chain_bridge/test_nats_tx.py::test_nats_submit_signs_and_lands`). Hand-builds an inert-but-valid tx in Python (solders): one ComputeBudget `SetComputeUnitLimit` instruction, fee-payer = the dev signing pubkey `EzudwoHvNPAc4dpPi5ndU8MEZVHVzq3Pj3Thm9ooKmiJ` (= `SOLANA_PAYER_KEY`, the insecure-mode `InsecureKeypairProvider` key — the bridge attaches its sig at the fee-payer slot so the payer MUST equal it), a real recent blockhash read via Chain Bridge `GetLatestBlockhash`. bincode-serializes it (solders legacy `Transaction` wire form IS bincode → matches the bridge `bincode::deserialize::<Transaction>`), wraps it in the full `TxSubmitMessage` JSON envelope (`nats_schema.rs`: `serialized_tx` as a u8 array, `key_id="platform_admin"`, mappable SPIFFE `service_identity`, fresh `created_at_ms`, unique `correlation_id`/`reply_subject`; `auth` omitted — `CHAIN_BRIDGE_REQUIRE_SIGNED_NATS=false` accepts log-only), publishes to `chain.tx.submit` over **core NATS** (host port 9020→4222) and awaits the bridge's reply on `chain.tx.result.{cid}`. Asserts `success` + non-empty base58 `signature`, then **confirms the tx actually landed** via a TEST-ONLY direct `getSignatureStatuses` against the validator (real slot > 0, `err==null`) — the bridge's own result `slot` is hardcoded 0 (`service.rs:243`). New `lib/nats_util.py` (core NATS request/reply) + `chain.get_latest_blockhash`. Covers the ACK half of line 96. Live PASS (3.5s).
- [x] Simulate: `chain.tx.simulate` returns result, no land — **DONE 2026-06-14** (`...::test_nats_simulate_returns_result_no_land`). Same envelope/tx published to `chain.tx.simulate`, reply on `chain.tx.simulate.result.{cid}`. Asserts `success` + presence of `logs`/`compute_units_consumed`, and that the result carries **no `signature`** (`TxSimulateResultMessage` has no sig field → proof of no on-chain land). Live PASS.
...
> **~~Deferred~~ DONE 2026-06-14:** direct NATS submit/simulate now exercised live (`50_chain_bridge/test_nats_tx.py`) — hand-built bincode `Transaction` (solders, ComputeBudget no-op, dev-payer fee-payer) published to `chain.tx.submit`/`chain.tx.simulate`, bridge signs+lands / simulates, asserted via the reply subjects + a direct validator `getSignatureStatuses` land-confirm. RBAC still additionally covered via Rust invariants.
...
- [x] NATS JetStream path assert (submit + ack) — **DONE 2026-06-14** via `50_chain_bridge/test_nats_tx.py` (see Phase 3 line 76): publishing to `chain.tx.submit` and receiving the bridge's `chain.tx.result.{cid}` reply exercises the JetStream consume→sign→submit→ack→reply round-trip (the consumer publishes the result then `msg.ack()`s; a reply received ⇒ the message was consumed + acked). Submit + ack proven; on-chain land confirmed independently.
...
> **Doc remark check:** corrected Phase 4 (generation-mint handler is in trading-service submodule, not solely gridtokenx-api). Verified still-accurate: IAM has no `/meters`; Chain Bridge binds 0.0.0.0; `settlements` table = trade (buyer/seller/epoch).
...
- [~] Gateways: APISIX routing + `GATEWAY_SECRET` enforcement; `:4000` health aggregate. *(80_gateways checks pass when up; gateways out-of-repo → skip loudly when down. Envoy IoT path removed 2026-06-14 — IoT ingresses direct to the Aggregator Bridge.)*
...
| 80_gateways | **3P / 0F** | clean — orchestrator `:4000`, APISIX `:4001` route+secret. _(Envoy `:4002` case removed 2026-06-14 with the edge.)_ |
