...
- [x] Envoy mTLS enforcement: non-mTLS client at `:4002` rejected — **DONE 2026-06-13** (was BLOCKED on the plaintext stub — TD-003). Rewrote `envoy_conf/envoy.yaml` as an mTLS listener (`require_client_certificate: true`, server cert + dev CA from `infra/certs/`, mounted RO via docker-compose) and recreated the container. `80_gateways/test_envoy_mtls.py` PASS (3/3): plaintext HTTP → rejected, HTTPS without client cert → handshake fails, HTTPS with a CA-signed client cert → `200 "ok"`. Enforcement half closed; the authenticated request still hits a `direct_response` stub, not the real Aggregator IoT upstream (routing half of TD-003 stays open).
