...
- [ ] Envoy mTLS enforcement: non-mTLS client at `:4002` rejected — **BLOCKED on missing mTLS config** (2026-06-13 verified: the only in-tree Envoy config `envoy_conf/envoy.yaml` is a self-declared dev *stub* — a plaintext HTTP listener with a `direct_response: 200 "ok"`, no `transport_socket`/`require_client_certificate`. Live probe: `http://localhost:4002/`→`200 server:envoy`, `https://…`→`000` (TLS bytes hitting a plaintext port, NOT mTLS rejection). The test can't assert mTLS rejection until a real edge config lands — see TD-003. Unblock = write the Envoy mTLS listener (CA + `require_client_certificate`), then this becomes a plain "plaintext/clientless-TLS client → connection refused" assertion.)
