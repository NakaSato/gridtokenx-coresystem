...
- [ ] **Matcher never promotes full fills to `Filled`.** `matcher_service.rs:141,148` hardcode `OrderStatus::PartiallyFilled` in the order-delta map and never compare cumulative fill vs order size — a fully-filled order (filled_amount == energy_amount) stays `partially_filled`. Affects settlement/refund status semantics.
...
- [ ] **`outbox_events` table missing** → `OutboxWorker` errors every 5s (`relation "outbox_events" does not exist`). Transactional-outbox events for the trade pipeline are dropped.
